...
| `m_low_limit_id` | 创建 ReadView 时已分配的最大事务 ID 加 1（作为"未来事务"的截止线）|
...
如果 DB_TRX_ID < m_low_limit_id AND DB_TRX_ID NOT IN m_ids:
    → 该事务在 ReadView 创建前已提交，当前版本可见

如果 DB_TRX_ID >= m_low_limit_id:
    → 该事务在 ReadView 创建之后才分配 ID（未来事务），不可见，沿 DB_ROLL_PTR 找上一个版本
...
InnoDB 表的数据按主键顺序存储，每个表都有一种特殊的索引结构——聚集索引和辅助索引。

**聚集索引（Clustered Index）**：
- 表数据按主键顺序存储在 B+Tree 叶节点中
- 叶节点存储完整的行数据（所有列的值）
- 主键查询只需一次 I/O（沿 B+Tree 从根到叶）
- 每张表只能有一个聚集索引（主键唯一确定）

**辅助索引（Secondary Index）**：
- 辅助索引的叶节点存储索引列值和对应的主键值，而非完整行数据
- 查询时先在辅助索引中找到主键值，再回表到聚集索引查找完整行
- 一张表可以有多个辅助索引
...
| 区别 | 聚集索引 | 辅助索引 |
|------|---------|---------|
| 叶节点存储内容 | 完整行数据 | 索引列值 + 主键值 |
| 查询方式 | 直接获取数据 | 先辅助索引，再回表 |
| 主键查询 I/O | 1 次 | 2 次（除非覆盖索引） |
| 数量 | 每表只能有 1 个 | 每表可以有多个 |
...
InnoDB 由内存结构和磁盘结构组成。本节详细介绍这两种结构。
...
如果没有老区/年轻区之分，全表扫描会把所有扫描过的页放入 LRU 链表前端，导致热点数据被挤出缓冲池。划分后，扫描的页进入老区，如果只是偶尔访问一次就会被淘汰，热点数据得以保留在年轻区。
...
1. Hash 表快速查找：页在缓冲池吗？（O(1) 复杂度）
   ├─ 命中 → 返回数据给用户
   │         └─ 如果该页在 Old 区，且被多次访问 → 移入 Young 区
   │
   └─ 未命中 → 从磁盘加载页 → 插入 Old head → 返回
```
...
| 情况 | 结果 |
|------|------|
| 页在 Young 区命中 | 直接返回（热数据） |
| 页在 Old 区命中 | 返回数据，如果多次访问则移入 Young 区 |
| 页未命中，缓冲池未满 | 从磁盘加载 → 插入 Old head → 返回 |
| 页未命中，缓冲池已满 | 淘汰 Old tail → 加载新页 → 插入 Old head → 返回 |
...
次级索引（Secondary Index）= 除主键索引以外的所有索引。

| 索引类型 | 说明 | 示例 |
|---------|------|------|
| **主键索引** | 主键列上的索引，数据按主键排序存储 | `PRIMARY KEY(id)` |
| **次级索引** | 非主键列上的索引 | `INDEX(name)`, `INDEX(age)` |
...
CREATE TABLE users (
    ...
    name VARCHAR(50),          -- 次级索引
    age INT,                   -- 次级索引
    email VARCHAR(100)         -- 次级索引
);
```

- `id` 上的索引 → **主键索引（聚集索引）**
- `name`、`age`、`email` 上的索引 → **次级索引**

**为什么叫"次级"？**

因为次级索引的叶节点**不存储完整行数据**，只存储 `(列值, 主键值)`。查询时需要回表（查聚集索引）才能获取完整数据。
...
#### 4.1.3 自适应哈希索引（Adaptive Hash Index）
...
##### 每表独立表空间（File-Per-Table Tablespace）
