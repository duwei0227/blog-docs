...
### 重音（Accent）与假名（Kana）的含义

**重音（Accent）**指语言中的变音符号，即附加在字母上的发音标记。常见于欧洲语言，如德语 `ä`、`ö`、`ü`、`ß`，法语 `é`、`è`，西班牙语 `ñ` 等。

- `_ai`（Accent Insensitive）：忽略重音差异，`ä` 与 `a` 等价比较
- `_as`（Accent Sensitive）：区分重音差异，`ä` 与 `a` 不等价

**假名（Kana）**是日语的两套音节文字系统——圆润的平假名（にほんご）和方正的片假名（ニホンゴ）。同一发音可对应两种写法。

- 无 `_ks` 后缀：平假名和片假名在排序时视为相等
- `_ks`（Kana Sensitive）：区分平假名和片假名，排序时视为不同字符
...
```sql
SET NAMES utf8mb4;
SELECT HEX('に') AS hira_he, HEX('ニ') AS kata_ni;
SELECT 'にほんご' COLLATE utf8mb4_ja_0900_as_cs;
SELECT 'ニホンゴ' COLLATE utf8mb4_ja_0900_as_cs;
SELECT 'に' = 'ニ' COLLATE utf8mb4_ja_0900_as_cs;
SELECT 'に' = 'ニ' COLLATE utf8mb4_ja_0900_as_cs_ks;
```
...
> 平假名 `に`（`E381AB`）和片假名 `ニ`（`E3838B`）编码不同。无 `_ks` 时视为相等，有 `_ks` 时区分。
