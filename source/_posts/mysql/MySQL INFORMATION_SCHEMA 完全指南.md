...
| 线程池表 | 线程池插件专用（已弃用，建议迁移到 Performance Schema） |
| 连接控制表 | `CONNECTION_CONTROL` 插件专用 |
| 企业防火墙表 | MySQL Enterprise Firewall 专用 |

本机 MySQL 8.4.8 环境中的 `INFORMATION_SCHEMA` 表数量验证如下：
...
> 💡 在本机 `mysql` 系统数据库中没有用户自定义外键，因此该查询在系统表上返回空结果。在实际业务数据库中，可将 `CONSTRAINT_SCHEMA` 替换为实际数据库名进行查询。
...
> 💡 本机 MySQL 实例中系统事件调度器（Event Scheduler）默认关闭，因此  数据库中没有预设事件。以上查询返回空结果。
...
> ⚠️ 注意：以下 `INFORMATION_SCHEMA` 线程池表已在 MySQL 8.4 中弃用，建议迁移到 Performance Schema 的对应表。

| 弃用表 | 建议迁移 |
|--------|---------|
| `TP_THREAD_GROUP_STATE` | `performance_schema.tp_thread_group_state` |
| `TP_THREAD_GROUP_STATS` | `performance_schema.tp_thread_group_stats` |
| `TP_THREAD_STATE` | `performance_schema.tp_thread_state` |

# 六、连接控制表

MySQL Enterprise 的 `CONNECTION_CONTROL` 插件通过 `INFORMATION_SCHEMA` 提供连接失败尝试的监控。

### CONNECTION_CONTROL_FAILED_LOGIN_ATTEMPTS 表

该表记录了每个账户当前连续失败的连接尝试次数。

关键列说明：

- `USERHOST`：账户名（格式为 `'user_name'@'host_name'`）
- `FAILED_ATTEMPTS`：当前连续失败次数

> 💡 注意：`CONNECTION_CONTROL` 插件必须激活此表才可用。当账户成功连接后，失败计数会重置为零。
...
MySQL Enterprise Firewall 提供应用层防护，通过以下 `INFORMATION_SCHEMA` 表提供防火墙配置信息。这些表是防火墙内存数据缓存的视图，需要相应的防火墙插件启用才能使用。

> ⚠️ 注意：以下防火墙表已在 MySQL 8.4 中弃用。

### MYSQL_FIREWALL_USERS 表

记录防火墙账户配置文件的名称和当前运行模式。

关键列说明：

- `USERHOST`：账户名（格式为 `'user_name'@'host_name'`）
- `MODE`：运行模式（`OFF`、`DETECTING`、`PROTECTING`、`RECORDING`、`RESET`）

### MYSQL_FIREWALL_WHITELIST 表

记录防火墙账户配置文件的放行规则（白名单）。
...
在 MySQL 8.4 中，部分早期 `INFORMATION_SCHEMA` 表（如 `PROCESSLIST`、`PROFILING`、线程池表、连接控制表等）已标记为弃用或已弃用，官方建议迁移到 Performance Schema 的对应实现。InnoDB 相关的 `INFORMATION_SCHEMA` 表是诊断 InnoDB 内部行为的重要工具，尤其在分析锁等待、长事务等问题时不可或缺。
