---
title: 设计模式之适配器模式
published: false
layout: post
date: 2025-12-08 18:40:00
permalink: /design_pattern/adaptor_pattern.html
categories: [设计模式]

---



## 概念

**适配器模式**是一种结构型设计模式，它允许接口不兼容的类能够一起工作。适配器模式就像现实生活中的电源适配器，可以让不同标准的插头连接到插座上。



**核心思想**：将一个类的接口转换成系统期望的另一个接口，使原本接口不兼容的类可以一起工作



## 适用场景

**✅ 适用场景：**

1. **整合第三方库**：需要使用现有的类，但其接口不符合需求
2. **系统升级兼容**：新系统需要兼容旧系统的接口
3. **统一多个类接口**：多个类有相似功能但接口不同
4. **复用遗留代码**：老代码功能可用但接口不兼容



**使用建议**

- 适配器模式应谨慎使用，特别是在详细设计阶段，它更多地用于解决现有系统的问题。
- 在考虑修改一个正常运行的系统接口时，适配器模式是一个合适的选择。



**🎯 典型例子：**

- **电压转换器**：220V转110V
- **读卡器**：SD卡读卡器、TF卡读卡器
- **日志框架适配**：Log4j、SLF4J、Logback
- **数据库驱动**：JDBC驱动适配不同数据库
- **媒体播放器**：播放不同格式文件



## 类图

![image-20251208185108053](https://raw.githubusercontent.com/duwei0227/picbed/main/blogs/image-20251208185108053.png)

**类图说明：**

- **`Client`**：客户端，使用Target接口
- **`Target`**：目标接口，客户端期望的接口
- **`Adaptee`**：被适配者，已有的但不兼容的类
- **`Adapter`**：适配器，将Adaptee接口转换为Target接口



## 示例

### 1、目标类

```java
package cn.probiecoder.adaptor;

// 目标接口：中国标准插座
public interface ChinaSocket {
    void supplyPower();  // 提供220V电源
}

```





### 2、被适配类

```java
package cn.probiecoder.adaptor;

// 被适配者：美国电器（需要110V电源）
public class AmericanAppliance {
    private String name;
    
    public AmericanAppliance(String name) {
        this.name = name;
    }
    
    // 美国电器的工作方法（需要110V）
    public void workWith110V() {
        System.out.println(name + " 正在使用110V电压工作");
    }
    
    public String getName() {
        return name;
    }
}
```



### 3、适配器

```java
package cn.probiecoder.adaptor;

// 适配器：电压转换器（将220V转换为110V）
public class VoltageAdapter implements ChinaSocket {
    private AmericanAppliance appliance;
    
    public VoltageAdapter(AmericanAppliance appliance) {
        this.appliance = appliance;
    }
    
    @Override
    public void supplyPower() {
        System.out.println("适配器：将220V转换为110V");
        System.out.println("为 " + appliance.getName() + " 供电...");
        appliance.workWith110V();
    }
    
    // 其他可能的方法
    public void checkVoltage() {
        System.out.println("检查输入电压：220V");
        System.out.println("输出电压：110V");
    }
}
```



### 4、客户端

```java
package cn.probiecoder.adaptor;

// 客户端：在中国使用美国电器
public class Client {
    public static void main(String[] args) {
        System.out.println("=== 电源适配器示例 ===");
        
        // 创建美国电器
        AmericanAppliance macbook = new AmericanAppliance("MacBook Pro");
        AmericanAppliance iphone = new AmericanAppliance("iPhone");
        
        // 创建适配器
        ChinaSocket adapter1 = new VoltageAdapter(macbook);
        ChinaSocket adapter2 = new VoltageAdapter(iphone);
        
        // 在中国插座上使用美国电器
        System.out.println("\n使用MacBook：");
        adapter1.supplyPower();
        
        System.out.println("\n使用iPhone：");
        adapter2.supplyPower();
        
        // 也可以直接使用适配器的其他功能
        System.out.println("\n检查电压转换：");
        ((VoltageAdapter) adapter1).checkVoltage();
    }
}
```

