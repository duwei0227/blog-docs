---
title: 设计模式之建造者模式
published: false
layout: post
date: 2025-11-25 11:28:07
permalink: /design_pattern/builder_pattern.html
categories: [设计模式]

---


## 

## 概念

将一个复杂对象的构建与其表示分离，使得同样的构建过程可以创建不同的表示。



解决复杂对象创建的问题的方式：

- 将复杂对象的构建过程分解为多个简单的步骤
- 通过一个指导者（Director）来控制构建过程
- 不同的建造者可以实现相同的构建步骤，但产生不同的产品



同一个产品，构建过程复杂，可以由多个独立的步骤组成，且步骤的组合顺序，组合数量的不同产生了同类产品的不同型号。



## 适用场景

**✅ 适用场景**

1. **创建复杂对象**：对象有很多组成部分，创建过程复杂，且各个部分可能需要不同的创建逻辑
2. **创建过程需要多个步骤**：对象的创建需要多个步骤，且这些步骤的顺序很重要
3. **需要创建不同的对象表示**：使用相同的创建过程，但需要生成不同的产品变体
4. **避免重叠的构造函数（telescoping constructor）**：当构造函数参数过多时，使用建造者模式可以提高代码可读性



**🎯 典型例子：**

- 电脑配置（CPU、内存、硬盘、显卡等的不同组合）
- 汉堡套餐（面包、肉饼、蔬菜、酱料的不同组合）
- 房屋建造（地基、墙体、屋顶、装修的不同风格）
- 文档生成器（标题、正文、页眉、页脚的不同格式）



## 类图

![image-20251127085745314](https://raw.githubusercontent.com/duwei0227/picbed/main/blogs/image-20251127085745314.png)

**类图说明：** 
- **`Client`（客户端）**：使用指导者和建造者来创建产品
- **`Director`（指导者）**：知道构建过程的步骤，控制构建顺序，主要用于最终产品的构建，并根据不同的构建过程返回不同的Product
- **`Builder`（抽象建造者）**：定义构建产品的各个步骤
- **`ConcreteBuilder`（具体建造者）**：实现构建步骤，负责组装产品的各个部分
- **`Product`（产品）**：最终要创建的复杂对象



## 示例

### 1、定义产品

```java
package cn.probiecoder.builder;

// 产品类 - 电脑
public class Computer {
    private String cpu;
    private String memory;
    private String storage;
    private String graphicsCard;
    private String display;
    
    public void setCpu(String cpu) {
        this.cpu = cpu;
    }
    
    public void setMemory(String memory) {
        this.memory = memory;
    }
    
    public void setStorage(String storage) {
        this.storage = storage;
    }
    
    public void setGraphicsCard(String graphicsCard) {
        this.graphicsCard = graphicsCard;
    }
    
    public void setDisplay(String display) {
        this.display = display;
    }
    
    @Override
    public String toString() {
        return "Computer配置:\n" +
               "  CPU: " + cpu + "\n" +
               "  内存: " + memory + "\n" +
               "  存储: " + storage + "\n" +
               "  显卡: " + graphicsCard + "\n" +
               "  显示器: " + display;
    }
}

```



### 2、定义Builder

```java
package cn.probiecoder.builder;

// 抽象建造者
public abstract class ComputerBuilder {
    protected Computer computer;
    
    public ComputerBuilder() {
        this.computer = new Computer();
    }
    
    public abstract void buildCpu();
    public abstract void buildMemory();
    public abstract void buildStorage();
    public abstract void buildGraphicsCard();
    public abstract void buildDisplay();
    
    public Computer getResult() {
        return computer;
    }
}

package cn.probiecoder.builder;

// 游戏电脑建造者
public class GamingComputerBuilder extends ComputerBuilder {
    @Override
    public void buildCpu() {
        computer.setCpu("Intel i9-13900K");
    }
    
    @Override
    public void buildMemory() {
        computer.setMemory("32GB DDR5");
    }
    
    @Override
    public void buildStorage() {
        computer.setStorage("2TB NVMe SSD");
    }
    
    @Override
    public void buildGraphicsCard() {
        computer.setGraphicsCard("NVIDIA RTX 4090");
    }
    
    @Override
    public void buildDisplay() {
        computer.setDisplay("32寸 4K 144Hz");
    }
}


package cn.probiecoder.builder;


// 办公电脑建造者
public class OfficeComputerBuilder extends ComputerBuilder {
    @Override
    public void buildCpu() {
        computer.setCpu("Intel i5-12400");
    }
    
    @Override
    public void buildMemory() {
        computer.setMemory("16GB DDR4");
    }
    
    @Override
    public void buildStorage() {
        computer.setStorage("512GB SSD");
    }
    
    @Override
    public void buildGraphicsCard() {
        computer.setGraphicsCard("集成显卡");
    }
    
    @Override
    public void buildDisplay() {
        computer.setDisplay("24寸 1080P");
    }
}
```



### 3、定义Director

```java
package cn.probiecoder.builder;

// 指导者
public class ComputerDirector {
    private ComputerBuilder builder;
    
    public void setBuilder(ComputerBuilder builder) {
        this.builder = builder;
    }

    public ComputerDirector() {
    }

    public ComputerDirector(ComputerBuilder builder) {
        this.builder = builder;
    }
    
    // 标准构建过程
    public Computer constructStandardComputer() {
        builder.buildCpu();
        builder.buildMemory();
        builder.buildStorage();
        builder.buildGraphicsCard();
        builder.buildDisplay();
        return builder.getResult();
    }
    
    // 简化构建过程（不带显示器）
    public Computer constructComputerWithoutDisplay() {
        builder.buildCpu();
        builder.buildMemory();
        builder.buildStorage();
        builder.buildGraphicsCard();
        // 不构建显示器
        return builder.getResult();
    }
}
```



### 4、调用方Client

```java
package cn.probiecoder.builder;

// 客户端
public class Client {
    public static void main(String[] args) {
        // 创建指导者
        ComputerDirector director = new ComputerDirector();
        
        // 构建游戏电脑
        ComputerBuilder gamingBuilder = new GamingComputerBuilder();
        director.setBuilder(gamingBuilder);
        
        Computer gamingComputer = director.constructStandardComputer();
        System.out.println("=== 游戏电脑 ===");
        System.out.println(gamingComputer);
        System.out.println();
        
        // 构建办公电脑
        ComputerBuilder officeBuilder = new OfficeComputerBuilder();
        director.setBuilder(officeBuilder);
        Computer officeComputer = director.constructStandardComputer();
        System.out.println("=== 办公电脑 ===");
        System.out.println(officeComputer);
        
        System.out.println();
        
        // 构建不带显示器的办公电脑
        ComputerBuilder officeBuilder2 = new OfficeComputerBuilder();
        director.setBuilder(officeBuilder2);
        Computer officeComputer2 = director.constructComputerWithoutDisplay();
        System.out.println("=== 不带显示器的办公电脑 ===");
        System.out.println(officeComputer2);
    }
}
```



